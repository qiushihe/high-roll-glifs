...
import { Tooltip, showTooltip, tooltips } from "@codemirror/tooltip";
import { syntaxTree } from "@codemirror/language";

class MarkdownInspector {
  private static defaultInstance = null;

  static getDefaultInstance(): MarkdownInspector {
    if (this.defaultInstance === null) {
      this.defaultInstance = new MarkdownInspector();
    }
...
const inspectorStateField = StateField.define<readonly Tooltip[]>({
  create: MarkdownInspector.getDefaultInstance().getTooltips,

  update(tooltips, transaction) {
    if (!transaction.docChanged && !transaction.selection) {
      return tooltips;
    } else {
      return MarkdownInspector.getDefaultInstance().getTooltips(
        transaction.state
      );
    }
  },
...
const inspectorTheme = EditorView.baseTheme({
